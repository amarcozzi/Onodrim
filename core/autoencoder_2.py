...
def train_autoencoder(plot_data, feature_cols, feature_weights=None, latent_dim=32, hidden_dims=(128, 96),
                      batch_size=64, learning_rate=0.001, num_epochs=150, dropout_rate=0.2):
    """
    ...
    model = ForestAutoencoder(input_dim, latent_dim=latent_dim, hidden_dims=hidden_dims, dropout_rate=dropout_rate)
...
def visualize_metric_performance(predictions_df, metric, plots_dir, model_name):
    ...
    if metric != 'FORTYPCD':
        ...
        # Add lines for 1 and 2 standard deviations
        std_dev = predictions_df[f'error_{metric}'].std()
        ax2.axvline(x=std_dev, color='g', linestyle='--', label='1 Std Dev')
        ax2.axvline(x=-std_dev, color='g', linestyle='--')
        ax2.axvline(x=2 * std_dev, color='b', linestyle='--', label='2 Std Dev')
        ax2.axvline(x=-2 * std_dev, color='b', linestyle='--')

        ax2.set_title(f'Error Distribution for {metric}\nMean error: {predictions_df[f"error_{metric}"].mean():.4f}, Std: {predictions_df[f"error_{metric}"].std():.4f}')
...
def main():
    ...
    model, scaler, X_test, y_test, plot_data, feature_cols = train_autoencoder(
        ...
        hidden_dims=[32],  # Residual block dimensions
        batch_size=128,
        learning_rate=0.001,
    ...
    evaluation = evaluate_comprehensive(
        ...
        model_name="ResidualAutoencoder"
    )
    ...
    }, os.path.join(data_dir, 'residual_autoencoder_model.pt'))

    print(f"Model saved as {os.path.join(data_dir, 'residual_autoencoder_model.pt')}")
