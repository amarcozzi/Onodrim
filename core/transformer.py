...
class MultiHeadAttention(nn.Module):
    """Multi-Head Attention module following modern transformer designs"""
    ...
    def forward(self, x):
        ...
        # Since we're dealing with tabular data, we need to reshape to include a sequence dimension
        x_seq = x.unsqueeze(1)  # [batch_size, 1, input_dim]

        # Linear projections
        q = self.query(x_seq)  # [batch_size, 1, input_dim]
        k = self.key(x_seq)  # [batch_size, 1, input_dim]
        v = self.value(x_seq)  # [batch_size, 1, input_dim]

        # Reshape for multi-head attention
        # Split input_dim into num_heads x head_dim
        q = q.view(batch_size, 1, self.num_heads, self.head_dim).permute(0, 2, 1,
                                                                         3)  # [batch_size, num_heads, 1, head_dim]
        k = k.view(batch_size, 1, self.num_heads, self.head_dim).permute(0, 2, 1,
                                                                         3)  # [batch_size, num_heads, 1, head_dim]
        v = v.view(batch_size, 1, self.num_heads, self.head_dim).permute(0, 2, 1,
                                                                         3)  # [batch_size, num_heads, 1, head_dim]

        # Compute attention scores
        # Attention shape: [batch_size, num_heads, 1, 1]
        ...
        # Reshape back: [batch_size, 1, input_dim]
...
class FeedForward(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=None, dropout=0.1):
        ...
        if hidden_dim is None:
            hidden_dim = input_dim * 4  # Standard transformer uses 4x expansion

        self.net = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.GELU(),  # Modern transformers typically use GELU
            nn.Dropout(dropout),
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, input_dim, num_heads=4, dropout=0.1, ff_dim=None):
...
class ForestTransformerAutoencoder(nn.Module):
    def __init__(self, input_dim, latent_dim=64, hidden_dims=(128, 96),
                 num_blocks_per_level=2, num_heads=4, dropout_rate=0.1):
        super(ForestTransformerAutoencoder, self).__init__()
        ...
        # Process each hidden dimension level with its own set of transformer blocks
        for i, dim in enumerate(hidden_dims):
            # First project to the current hidden dimension
            encoder_layers.append(nn.Linear(current_dim, dim))
            current_dim = dim

            # Add the specified number of transformer blocks at this dimension level
            for _ in range(num_blocks_per_level):
                encoder_layers.append(TransformerBlock(
                    input_dim=current_dim,
                    num_heads=num_heads,
                    dropout=dropout_rate,
                    ff_dim=current_dim * 4  # Standard transformer practice
                ))

        # Final projection to latent space
        encoder_layers.append(nn.LayerNorm(current_dim))
        encoder_layers.append(nn.Linear(current_dim, latent_dim))

        # Create encoder sequential model
        self.encoder = nn.Sequential(*encoder_layers)

        # Decoder layers
        decoder_layers = []

        # Initial projection from latent space to last hidden dimension
        current_dim = latent_dim
        if hidden_dims:
            decoder_layers.append(nn.Linear(current_dim, hidden_dims[-1]))
            current_dim = hidden_dims[-1]

        # Process each hidden dimension in reverse order
        for i in range(len(hidden_dims) - 1, -1, -1):
            # Current dimension is already set from previous step
            dim = hidden_dims[i]

            # Add transformer blocks for this dimension level
            for _ in range(num_blocks_per_level):
                decoder_layers.append(TransformerBlock(
                    input_dim=current_dim,
                    num_heads=num_heads,
                    dropout=dropout_rate,
                    ff_dim=current_dim * 4
                ))

            # Project to next dimension (or input dimension if at the end)
            if i > 0:
        ...
        # Final normalization
        decoder_layers.append(nn.LayerNorm(current_dim))

        # Create decoder sequential model
...
def train_transformer_autoencoder(plot_data, feature_cols, feature_weights=None, latent_dim=32,
                                  hidden_dims=(128, 96), num_blocks_per_level=2, num_heads=4,
                                  batch_size=64, learning_rate=0.001, num_epochs=150, dropout_rate=0.1):
    """
    ...
    Args:
        ...
        hidden_dims: Tuple of hidden dimensions for transformer blocks
        num_blocks_per_level: Number of transformer blocks for each dimension level
    ...
    model = ForestTransformerAutoencoder(
        input_dim=input_dim,
    ...
    print(f"  Transformer blocks per level: {num_blocks_per_level}")
    print(f"  Total transformer blocks: {num_blocks_per_level * len(hidden_dims) * 2}")  # Encoder + decoder
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, 'transformer_training_curve.png'))
    plt.close()
...
def visualize_embeddings(model, scaler, plot_data, feature_cols, save_path='embeddings_tsne.png'):
    ...
    for key, metric_info in metrics.items():
        ...
        save_name = f'transformer_embeddings_tsne_{key}.png'
    ...
    plt.suptitle('t-SNE Visualization of Forest Plot Embeddings (Transformer)', fontsize=20)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(os.path.join(plots_dir, 'transformer_embeddings_tsne_combined.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def visualize_prediction_results(model, scaler, X_test, y_test, plot_data, feature_cols,
                                 plots_dir="./plots", model_name="TransformerAutoencoder", top_k=3):
    """
...
def evaluate_comprehensive(model, scaler, X_test, y_test, plot_data, feature_cols, model_name="TransformerAutoencoder"):
    """
...
def main():
    ...
    feature_weights = {
        ...
        'FORTYPCD': 3.0,  # Important for forest type matching
    ...
    hidden_dims = [16, 16, 16]  # Simpler architecture
    model, scaler, X_test, y_test, plot_data, feature_cols = train_transformer_autoencoder(
        plot_data,
        feature_cols,
        feature_weights=feature_weights,
        latent_dim=latent_dim,
        hidden_dims=hidden_dims,
        num_blocks_per_level=2,  # Number of transformer blocks per dimension level
        num_heads=4,  # Number of attention heads
        batch_size=128,
        learning_rate=0.001,
        num_epochs=1000,  # Allow longer training with early stopping
        dropout_rate=0.1
    )
    ...
    }, os.path.join(data_dir, 'transformer_autoencoder_model.pt'))

    print(f"Model saved as {os.path.join(data_dir, 'transformer_autoencoder_model.pt')}")
